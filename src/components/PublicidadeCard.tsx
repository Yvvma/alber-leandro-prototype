...
const PublicidadeCard = ({
  ...
  // Debug focus changes
  useEffect(() => {
    console.log(`PublicidadeCard ${firstTitle} focus changed:`, isFocused);
  }, [isFocused, firstTitle]);
  ...
  const card = (
    <motion.div
      ...
      className="p-4 md:p-8 lg:p-16 rounded-xl relative cursor-pointer"
    >
      <motion.div
        whileHover={{ scale: 1.05 }}
        transition={{ duration: 0.2 }}
        className={`relative w-[300px] md:w-[360px] aspect-[3/4] mx-auto overflow-hidden 
      ...
      >
        <video
          ...
          onError={() =>
            console.error('Preview video failed to load:', video, 'isFocused:', isFocused)
          }
        />
        ...
        <motion.div
          ...
          className="absolute top-2 right-2 flex flex-row items-center justify-center z-20"
        >
          <div className="flex flex-row items-center justify-center gap-2">
            <img src="/logos/play-circle_yvrfk5.svg" alt="Play Icon" className="w-4" />
        ...
        {platformIcon && (
          <img
            src={platformIcon}
            alt="Platform icon"
            className="max-w-[64px] md:max-w-[128px] absolute bottom-4 left-4 z-50"
          />
        )}
      ...
      <div className="flex flex-col gap-2 pt-4 pl-2">
        <h2 className="font-helvetica font-[200] uppercase text-sm md:text-base">{singleTitle}</h2>
        <h1 className="text-white text-2xl md:text-3xl lg:text-4xl font-helvetica font-[800]">
          {firstTitle}
        </h1>
      </div>
    </motion.div>
